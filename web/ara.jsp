...
<body>
    ...
    <ul>
        <%String encodedBilgi = request.getParameter("bilgi");
        String gelenBilgi = java.net.URLDecoder.decode(encodedBilgi, "UTF-8");%>
    ...
    <% for (int i = 0; i < rowCount; i++) { %>
        <div class="book-card">
            ...
            <div class="book-card-content">
                <a id="adi" href="detay.jsp?bilgi=<%=kitaplar[i][j]%>">  
                <h3>
                <script>
                var maxCharacters = 12; // İstediğiniz karakter sayısını burada belirleyin
                var text = '<%= kitaplar[i][j] %>'; // Yazdırılacak veriyi alıyoruz

                if (text.length > maxCharacters) {
                text = text.slice(0, maxCharacters) + '...'; // İstenen uzunluğa kadar kesip üç nokta ekliyoruz
                }

                document.write(text); // Sonucu yazdırıyoruz
                </script>
            </h3>
